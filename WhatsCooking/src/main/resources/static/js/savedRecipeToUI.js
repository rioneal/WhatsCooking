/*function savedMap(recipe) {
	...
	$.each(recipe, function (i, field) {
		...
		console.log(field);
		if (count <= 1) {
			$("#savedList").append('<div class="row dynRecipe"><div class="col-lg-12 mb-12"><div class="card h-100"><h4 class="card-header">' + field.title + ' <small style="float: right">Preparation Time: <b>' + field.readyInMinutes + ' minutes</b></small> </h4> <div class="card-body"> <img src="' + field.image + '" height="100px" width="100px" style="float: left;"><p class="card-text">  Ingredients needed: ' + field.missedIngredientCount + ' </p><p class="card-text"> Servings: ' + field.servings + '</p> <p class="card-text"> Calories per serving: ' + field.calories + '  </p><p class="card-text"> Price per serving: $' + (field.pricePerServing/100).toFixed(2) + '</p> <input type="button" style="float: right" class="btn btn-md btn-primary" data-toggle="modal" data-target="#modal'+field.id+'" onclick="modalCreate('+field.id+')" value="View Recipe"></div></div></div> </div>');
